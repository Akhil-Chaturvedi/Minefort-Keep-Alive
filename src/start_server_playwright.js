...
(async () => {
  ...
  try {
    ...
    // Increased timeout for input fields presence
    ...
    // Increased timeout slightly for this check
    ...
    // Wait for the URL to change away from the login page after login attempt
    ...
    try {
        ...
        // Increased timeout for waiting for URL change
        ...
        if (currentUrlAfterLoginAttempt.startsWith(LOGIN_URL)) {
             ...
             let loginErrorText = "No specific error message found on page (after waiting for URL change).";
              if (await loginErrorElement.isVisible({timeout: 3000}).catch(() => false)) { // Increased timeout
                 loginErrorText = await loginErrorElement.textContent({timeout:2000}) || "Error message element found but was empty."; // Increased timeout
             }
    ...
    // to be off the login page, navigate to the specific server dashboard if not already there.
    ...
    // Increased timeout for buttons to appear
    ...
    // Increased timeout for wake up button visibility check
    if (await wakeUpButton.isVisible({ timeout: 15000 }).catch(() => false)) {
      ...
      console.log('Clicked "Wake up server". Waiting 30 seconds for state change...'); // Increased wait
    ...
    // Increased timeout significantly as server startup can take time
    ...
    const finalServerStartWait = 30 * 1000; // Wait 30 seconds after clicking start
    console.log(`Clicked Start. Waiting ${finalServerStartWait / 1000} seconds for server state to update...`);
