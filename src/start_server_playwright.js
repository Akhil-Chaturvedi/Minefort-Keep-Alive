...
def add_and_commit_backup(repo_dir, backup_filepath):
    ...
    try:
        ...
        try:
            ...
            current_branch = subprocess.run(['git', 'rev-parse', '--abbrev-ref', '--symbolic-full-name', 'HEAD'], capture_output=True, text=True, check=True).stdout.strip()
...
if __name__ == "__main__":
    ...
    try:
        ...
        with ftplib.FTP() as ftp:
             # Increased connection timeout significantly
             # This timeout applies to connect and subsequent commands like CWD, NLST, RETR initial handshake
             ftp.connect(FTP_HOST, FTP_PORT, timeout=300) # 5 minutes
